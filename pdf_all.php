...
function Row($data)
{
    ...
    foreach ($this->aCols as $col) {
        ...
        $alignment = ($col['a'] == 'J') ? 'J' : 'L'; // Corrected alignment determination

        // Output the text in the cell with correct alignment
        $this->MultiCell($col['w'], 5, $data[$col['f']], 1, $alignment, $fill);
    }
...
class PDF extends PDF_MySQL_Table
{
    ...
    {
        $this->SetFont('Arial', '', 12);
        $query = "SELECT material_requisition.QUANTITY, equip_pro.UNIT, equip_pro.EQUIP_DESCRIPTION, equip_pro.PROPERTY_NO, material_requisition.MR_DATE, equip_pro.COST FROM material_requisition JOIN equip_pro ON equip_pro.ID = material_requisition.EQUIP_ID JOIN program ON program.PROGRAM_ID = equip_pro.PROGRAM_ID WHERE program.TYPE = 'Equipment'";
